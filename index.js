...
const OFFICIAL_LOG_MATRIX = [
    "報告 ${ownerId}：系統已偵測到用戶 ${target} 具備違規行為，現已依規執行永久封鎖。",
    "報告 ${ownerId}：用戶 ${target} 試圖干擾群組秩序，系統已完成清理程序以維持環境安全。",
    "報告 ${ownerId}：針對用戶 ${target} 的異常活動，管理系統已即刻對其執行停權處分。",
    "報告 ${ownerId}：用戶 ${target} 因違反社群規範，目前已被移入黑名單並限制存取權限。",
    "報告 ${ownerId}：系統已成功排除用戶 ${target} 之異常干擾，該帳號已無法對本群組進行操作。",
    "報告 ${ownerId}：偵測到用戶 ${target} 嘗試發布不當內容，系統已採取強制退群處置。",
    "報告 ${ownerId}：為保障資安環境，系統已將惡意帳號 ${target} 移出通訊名單。",
    "報告 ${ownerId}：關於用戶 ${target} 的違規事證，系統已執行最終屏蔽指令並留存紀錄。",
    "報告 ${ownerId}：用戶 ${target} 的存取請求已被拒絕，系統已完成自動化排除程序。",
    "報告 ${ownerId}：偵測到用戶 ${target} 執行無效操作，系統已解除其所有連線並執行封鎖。",
    "報告 ${ownerId}：用戶 ${target} 涉及重複發布垃圾訊息，管理員權限已自動將其帳號註銷。",
    "報告 ${ownerId}：系統已排除用戶 ${target} 對本平台的干擾，相關帳號權限已全數凍結。",
    "報告 ${ownerId}：用戶 ${target} 試圖規避審核機制，系統已即時將其移除以維護運作穩定。",
    "報告 ${ownerId}：針對用戶 ${target} 的不當交互行為，系統已完成封鎖作業並通知受控端。",
    "報告 ${ownerId}：用戶 ${target} 已失去本群組之訪問資格，系統已將其權限變更為永久禁入。",
    "報告 ${ownerId}：為優化群組純淨度，系統已將高風險帳號 ${target} 列入攔截清單。",
    "報告 ${ownerId}：用戶 ${target} 的異常請求已遭系統攔截，該帳號目前已無法再次進入。",
    "報告 ${ownerId}：偵測到用戶 ${target} 頻繁洗版，系統已依照安全策略執行剔除操作。",
    "報告 ${ownerId}：用戶 ${target} 觸發了安全防護閾值，系統已自動將其從成員名單中移除。",
    "報告 ${ownerId}：針對特定帳號 ${target} 的管理處置已生效，系統運作已恢復正常。",
    "報告 ${ownerId}：用戶 ${target} 之違規事態已受控，系統已對該帳號實施無限期禁言處分。",
    "報告 ${ownerId}：系統紀錄顯示用戶 ${target} 具備潛在威脅，現已預防性終止其訪問權限。",
    "報告 ${ownerId}：用戶 ${target} 已因行為不合規被系統強制離線，相關數據已同步更新。",
    "報告 ${ownerId}：系統已完成對用戶 ${target} 的清理作業，該帳號將不再具備互動功能。",
    "報告 ${ownerId}：用戶 ${target} 之存取令牌已被系統吊銷，目前該帳號已無任何權限。",
    "報告 ${ownerId}：偵測到用戶 ${target} 具備不當企圖，系統已迅速響應並執行封鎖動作。",
    "報告 ${ownerId}：關於用戶 ${target} 的處置已執行完畢，系統已將其帳號狀態標記為失效。",
    "報告 ${ownerId}：用戶 ${target} 未通過安全驗證，系統已將其移除以確保群組數據安全。",
    "報告 ${ownerId}：針對帳號 ${target} 的自動化管理程序已結束，現已排除該用戶之所有連結。",
    "報告 ${ownerId}：用戶 ${target} 涉及干擾系統穩定性，目前已對其執行強制性退群處理。"
];
